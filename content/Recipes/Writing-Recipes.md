...
[NewVersion](/Commands/NewVersion), [EditRecipe](/Commands/EditRecipe)
...
you don't need to run [MakeRecipe](/Commands/MakeRecipe) again.
Instead, use [NewVersion](/Commands/NewVersion) with the package name
...
called "4.4.4" inside /Data/Compile/Recipes/GCC and will replace the
package version in "$url" by 4.4.4.
...
If the version of the package has not changed, and only minor recipe
updates are needed, [NewVersion](/Commands/NewVersion) will not work as
...
4.  Packages should never have spaces in their names. For example, use
    "AdobeReader" as the name, rather than "Adobe Reader" or
    "Adobe\_Reader".
5.  There should never be two package names differing only in
    capitalization. The GoboLinux scripts refer to package, program and
    recipe names in a case-insensitive manner.

The set of rules are defined in
[NamingConventions](/Commands/NamingConventions) and deal with
...
[NamingConventions](/Commands/NamingConventions). In recent versions,
...
In this example we'll make a recipe, starting from the source code for a
...
run of the [NamingConventions](/Commands/NamingConventions) script,
...
[MakeRecipe](/Commands/MakeRecipe) should now report that it has
...
the GoboLinux recipe, please consider contributing it to the community!
(See section above, under Writing Recipes for details.)
...
Request on GitHub so that the project maintainers can review it and
merge it. Please note that you will need a valid account at github.com
in order to contribute recipes.
...
and `---`, `--- rlocate-0.4.3/doc/rlocate.html 2006-01-19
