...
There are two ways to create a patch: you can do it manually or with our in-house
tool `AutoPatch`.
...
Let's create a patch for the `Nano 8.2` recipe.
 ...
 4. Verify the generated diff, set a name and description when prompted.
    ...
    "Fixes nano build on GCC v14 and above".
 5. Done! You'll find a file called `01-fix-gcc14-build.patch` autogenerated
    inside `/Data/Compile/Recipes/Nano/8.2/`.

#### Manually

To create a patch manually you need the edited source and the "clean" source,
then use `diff` to create the patch. To create the patch in the example above
I used
